...
// input: idx_c (b, n, ks) cid (b, m, 1)
// output: idx (b, m, n) cnt (b, m, 1)
__global__ void assomatrix_cuda_kernel(int b, int n, int m, int ks, const int *__restrict__ idx_c, const int *__restrict__ cid, int *__restrict__ idx, int *__restrict__ cnt) {
...
void assomatrix_cuda_launcher(int b, int n, int m, int ks, const int *idx_c, const int *cid, int *idx, int *cnt, cudaStream_t stream) {
    ...
    assomatrix_cuda_kernel<<<blocks, threads, 0, stream>>>(b, n, m, ks, idx_c, cid, idx, cnt);
